...
- 支持用 `EAGLE_CODEX_FOLDER_ID` 固定文件夹，自动化时可跳过 `folder_get` 权限确认
- 支持用 `EAGLE_CODEX_ROOT_FOLDER_ID` 固定根目录，同时继续按日期创建/复用子目录
- 按父子路径精确复用目录，并使用本地进程锁避免并发自动化重复创建日期目录
...
如果你想固定 `Codex 生成图` 这个根目录，但仍然按日期归档到它下面，使用根目录 ID：

```bash
node scripts/archive-codex-image-to-eagle.js --latest --root-folder-id "ROOT_FOLDER_ID" --prompt "完整提示词"
```

自动化任务里推荐用环境变量固定文件夹 ID。这样脚本不会调用 `folder_get` 或 `folder_create`，可以减少 MCP 权限弹窗：

```bash
export EAGLE_CODEX_FOLDER_ID="FOLDER_ID"
node scripts/archive-codex-image-to-eagle.js --latest --prompt "完整提示词"
```

如果自动化仍然需要日期子目录，推荐固定根目录 ID，而不是只传根目录名称：

```bash
export EAGLE_CODEX_ROOT_FOLDER_ID="ROOT_FOLDER_ID"
node scripts/archive-codex-image-to-eagle.js --latest --prompt "完整提示词"
```
